...
class GetWeeklyTimeGridUseCase {
        ...
        if (clientIds.isNotEmpty) {
          try {
            ...
            for (var i = 0; i < clientIds.length; i++) {
              ...
              if (i < users.length) {
                final u = users[i];
                clientNames[id] = (u['name'] as String?) ??
                    (u['displayName'] as String?) ??
                    (u['email'] as String?) ??
                    id;
              }
              clientNames.putIfAbsent(id, () => 'Cliente');
            }
          } catch (_) {
            for (final id in clientIds) {
              clientNames[id] = 'Cliente';
            }
          }
        }
      }
  ...
  /// Converte appointments em blocks. Usado por call() e stream().
  Future<List<TimeGridBlock>> _toBlocks(
  ...
  Stream<List<TimeGridBlock>> stream({
    ...
    final initial = await repository.getByProfessionalAndPeriod(
      professionalId: professionalId,
      start: startOfWeek,
      end: endOfWeek,
    );
